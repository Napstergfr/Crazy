...
$(function () {
    ...
    if ($('#paramOrderId').html() != "") {
        var orderId = Number($('#paramOrderId').html());
...
function showModalDialog(title, message, button1Caption, button2Caption, button1Function) {
...
function getFOW() {
    try {
        var date = $('#readyDateDiv').datetimepicker('date');
        if (date == null) {
...
function getIcings() {
    try {
        $.getJSON('https://abbygirlsweets.azurewebsites.net/data/icings')
            .done(function (data) {
            _icings = data;
            var none = new models.IdName(0, 'None');
            _icings.push(none);
            getToppings();
        })
            .fail(function (error) {
...
function checkBakingHours() {
    var date = $('#readyDateDiv').datetimepicker('date');
    var pickupDate = moment(date.format('YYYY-MM-DD'), 'YYYY-MM-DD');
    var message;
    ...
    var now = moment();
    var orderOnFriSatSun = (now.day() == 0 || now.day() >= 5);
    var pickupOnFriSatSun = (pickupDate.day() == 0 || pickupDate.day() >= 5);
...
function checkBusinessHours() {
    ...
    var pickupTime = moment('0001-01-01 ' + date.format('HH:mm'), 'YYYY-MM-DD HH:mm');
    var pickupDate = moment(date.format('YYYY-MM-DD'), 'YYYY-MM-DD');
    ...
    locations.forEach(function (l) {
        ...
        l.Closures.forEach(function (lc) {
...
function saveOrder() {
    try {
        ...
        $('.flavour').each(function () {
            var qty = Number($(this).find('input.flavour-qty').val());
            if (qty > 0) {
                _orderItems;
...
function validate() {
    ...
    var qty = Number($('#actualTotalQty').find('strong').html());
    if (qty == 0) {
        error += '<li>No cupcakes selected</li>';
    }
    var hasCakeError = false;
    $('.customitem').each(function () {
        var qty = Number($(this).find('input.flavour-qty').val());
        if (qty > 0) {
            var cake = $(this).find('select.cake').first();
            if (!cake.val()) {
...
function displayOrders(orders) {
    ...
    orders.forEach(function (order) {
        var trId = "order" + order.Id;
        var status = (order.Inactive) ? 'Cancelled' : 'Active';
...
function clearOrderDetails() {
    ...
    $('#deliveryState').val("");
    $('#deliveryZip').val("");
    $('#readyDateDiv').datetimepicker('date', null);
    $('#readyDate').attr('placeholder', 'Pickup Date/Time');
    $('#recipientPhone').val("");
    $('#recipientPhone').val("");
...
function displayOrderDetails(order) {
    ...
    $('#readyDateDiv').datetimepicker('date', _order.ForDate);
...
function pickupLocationName(location) {
    if (location == null) {
        return "";
    }
    else {
...
function displayOrderItems(items) {
    if (items == null)
        return;
    _orderItems = items;
    var otherFlavours = false;
    var customFlavours = false;
    var otherFlavoursHtml = '<div class="row">';
    var primaryFlavoursHtml = '<div class="row">';
    var customFlavoursHtml = '<div class="row bottommargin10"><div class="col-xs-3">Quantity</div>'
...
function displayCustom(item) {
    ...
    var display = '<div class="customitem" data-itemid="' + getId(item) + '"><div class="row bottommargin10">';
...
function displayItem(item) {
    ...
    var customized = (item.Customized == null || !item.Customized) ? "btn-default" : "btn-info";
    var inactive = (item.Inactive) ? "inactive-flavour" : "";
    var display = '<div id="flavour' + item.FlavourId + '" class="col-xs-2 flavour" '
...
function createSelect(items, selectedId, datatype, tag) {
    var select = '<select class="form-control selectpicker ' + datatype + ' ' + tag + '" title="Please select...">';
    var matched = false;
    items.forEach(function (item) {
...
function setFlavourClasses(otherFlavours, customFlavours) {
    if (otherFlavours) {
...
function attachCustomizePopup() {
    $(document.body).on('click', '.btn-customize', function () {
        var flavourId = $(this).parent().data('flavourid');
...
function attachNewCustomFlavourButton() {
    $(document.body).on('click', '#addCustomFlavour', function () {
        var display = displayCustom(null);
        $('#customFlavoursDiv').append(display);
        $('.selectpicker').selectpicker();
    });
    $(document.body).on('changed.bs.select', '.selectpicker.item', function (e) {
        var parent = $(this).closest('.flavour');
        var flavourId = parent.data('flavourid');
        var flavour = _orderItems.filter(function (item) { return item.FlavourId == flavourId; })[0];
        var toppingId;
        var icingId;
        var fillingId;
        ...
        var button = parent.find('.btn-customize').first();
